                ...
                <div className={`${gen?.aspectRatio === "9:16" ? "aspect-9/16" : "aspect-video"} relative overflow-hidden`}>
                    ...
                    {forCommunity && (
                        <div
                            ...
                            className="absolute right-3 top-3 sm:opacity-0 group-hover:opacity-100 transition flex items-center gap-3">
                            <div className="absolute top-3 right-3">
                                <EllipsisIcon className="ml-auto bg-blackl/10 rounded-full p-1 size-7" />
                            </div>
                            <div className="flex flex-col items-end w-32 text-sm">
                                <ul className={`text-xs ${menuOpen ? 'block' : 'hidden'} verflow-hidden right-0 peer-focus:block hover:block w-40 bg-black/50 backdrop:blur text-white border border-gray-500/50 rounded-lg shadow-md mt-2 py-1 z-10 `}>
                                    ...
                                    {gen.generatedImage && <a href={gen.generatedImage} download className="flex gap-2 items-center px-4 py-2 hover:bg-black/10 cursor-pointer"> <ImageIcon size={14} /> Download Image </a>
                                    }
                                    {gen.generatedVideo && <a href={gen.generatedVideo} download className="flex gap-2 items-center px-4 py-2 hover:bg-black/10 cursor-pointer"> <PlaySquareIcon size={14} /> Download Video </a>
                                    }
                ...
                <div className="p-4">
                    ...
                    <div className="flex items-start justify-between gap-4">
                        <div className="flex-1">
                            ...
                            {gen.updatedAt && (
                                <p className="text-sm text-gray-400">Updated {new Date(gen.updatedAt).toLocaleString()}</p>
                            )}
                        </div>
                    ...
                    {forCommunity && (
                        <div className="mt-4 grid grid-cols-2 gap-3">
                            ...
                            >
                                {gen.isPublished ? "Private" : "Public"}
                            </PrimaryButton>
                        </div>
                    )}
