...
const UploadZone: React.FC<UploadZoneProps> = ({ label, file, onClear, onChange }) => {
    return (
                ...
                {file ? (
                    ...
                    <>
                        <div className='w-16 h-16 rounded-full bg-white/5 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300'>
                            <UploadIcon className='w-8 h-8 text-gray-400 group-hover:text-violtet-400 transition-colors' />
                        </div>
                        <h3 className='text-lg font-semibold mb-2'>{label}</h3>
                        <p className='text-sm text-gray-400 text-center max-w-[200px]'>Drag & Drop or Click to upload</p>
                        <input type="file" name="" accept='image/*' onChange={onChange} className='absolute inset-0 w-full h-full opacity-0 cursor-pointer' />
                    </>
                )}
            </div>
        </div>
